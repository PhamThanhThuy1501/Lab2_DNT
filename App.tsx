...
export default function App() {
  ...
  const getapi = async () => {
    try {
      const res = await fetch('http://10.24.15.10:80/product')
  ...
  const themorsua=()=>{
    ...
    let method='POST';
    let url='http://10.24.15.10:80/product'
    if(update==true){
      method='PUT';
      url=`http://10.24.15.10:80/product/${select.id}`
    }
    fetch(url, {
      method: method,
      headers: {
    ...
    Alert.alert(`xóa công việc`,`Bạn có muốn xóa công việc ${item.title}`,[
      {
        text: 'ok',
      onPress:()=>{
        fetch(`http://10.24.15.10:80/product/${item.id}`, {
          method: 'DELETE',
        }).then((res)=>{
          if(res.status==200){
